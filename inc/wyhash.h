...
static inline uint32_t _wyr32(const uint8_t* p) { uint32_t v; memcpy(&v, p, 4); return v; }
#elif defined(__GNUC__) || defined(__INTEL_COMPILER) || defined(__clang__)
static inline uint32_t _wyr32(const uint8_t* p) { uint32_t v; memcpy(&v, p, 4); return __builtin_bswap32(v); }
#elif defined(_MSC_VER)
static inline uint32_t _wyr32(const uint8_t* p) { uint32_t v; memcpy(&v, p, 4); return _byteswap_ulong(v); }
#endif
static inline uint32_t _wyr24(const uint8_t* p, uint32_t k) { return (((uint32_t)p[0]) << 16) | (((uint32_t)p[k >> 1]) << 8) | p[k - 1]; }
